...
class Attention(nn.Module):
    def __init__(self, config: MiniMindConfig):
        ...
        self.flash = hasattr(torch.nn.functional, 'scaled_dot_product_attention') and config.flash_attn #境支持 FlashAttention2 + 配置 flash_attn
...
class MOEFeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, expert in enumerate(self.experts):
            mask = (topk_idx == i) #找出当前专家i对应的token 
            if mask.any():
                token_idx = mask.any(dim=-1).nonzero().flatten()  # 拿到所有命中当前专家的token下标，展平
                weight = topk_weight[mask].view(-1,1)
                y.index_add_(0, token_idx, (expert(x_flat[token_idx]) * weight).to(y.dtype))
            elif self.training:
                 # 空闲专家：添加零梯度损失，强制专家参数更新，防止死专家
                y[0, 0] += 0 * sum(p.sum() for p in expert.parameters())
            # load balancing loss
            if self.training and self.config.router_aux_loss_coef > 0:
                load = F.one_hot(topk_idx, self.config.num_experts).float().mean(dim=0) # 专家 e 实际被分配 token 的全局占比
...
class MiniMindModel(nn.Module):
    ...
    def forward(self, input_ids, attention_mask=None, past_key_values=None, use_cache=False, **kwargs):
        ...
        past_key_values = past_key_values or [None] * len(self.layers) # 空缓存 → 生成每层默认None占位
        start_pos = past_key_values[0][0].shape[1] if past_key_values[0] is not None else 0 # 计算当前序列起始位置
        hidden_states = self.dropout(self.embed_tokens(input_ids))
        if self.freqs_cos[0, 0] == 0: # 生成位置编码
        ...
        for layer, past_key_value in zip(self.layers, past_key_values):
            hidden_states, present = layer(
                hidden_states,
                position_embeddings,
                past_key_value=past_key_value, # 传入上一轮该层K/V
                use_cache=use_cache,
                attention_mask=attention_mask
            )
            presents.append(present) # 保存本层新K/V
...
class MiniMindForCausalLM(PreTrainedModel, GenerationMixin):
    ...
    def generate(self, inputs=None, attention_mask=None, max_new_tokens=8192, temperature=0.85, top_p=0.85, top_k=50, eos_token_id=2, streamer=None, use_cache=True, num_return_sequences=1, do_sample=True, repetition_penalty=1.0, **kwargs):
        input_ids = kwargs.pop("input_ids", inputs).repeat(num_return_sequences, 1) # 一条 prompt 复制 N 份
        attention_mask = attention_mask.repeat(num_return_sequences, 1) if attention_mask is not None else None
        past_key_values = kwargs.pop("past_key_values", None)
        finished = torch.zeros(input_ids.shape[0], dtype=torch.bool, device=input_ids.device) # 是否写完
        if streamer: streamer.put(input_ids.cpu())
        for _ in range(max_new_tokens):
            past_len = past_key_values[0][0].shape[1] if past_key_values else 0 # [layer, key/value, batch, heads, seq_len, head_dim]
            outputs = self.forward(input_ids[:, past_len:], attention_mask, past_key_values, use_cache=use_cache, **kwargs)
            attention_mask = torch.cat([attention_mask, attention_mask.new_ones(attention_mask.shape[0], 1)], -1) if attention_mask is not None else None # 注意力掩码需要在尾部增加一个长度
            logits = outputs.logits[:, -1, :] / temperature # T
            ...
            if top_p < 1.0: # 从top_k里选top_p
                sorted_logits, sorted_indices = torch.sort(logits, descending=True)
                mask = torch.cumsum(torch.softmax(sorted_logits, dim=-1), dim=-1) > top_p
                mask[..., 1:], mask[..., 0] = mask[..., :-1].clone(), 0
                logits[mask.scatter(1, sorted_indices, mask)] = -float('inf')
            next_token = torch.multinomial(torch.softmax(logits, dim=-1), num_samples=1) if do_sample else torch.argmax(logits, dim=-1, keepdim=True) # 生成
            if eos_token_id is not None: next_token = torch.where(finished.unsqueeze(-1), next_token.new_full((next_token.shape[0], 1), eos_token_id), next_token) # 根据finished是否取
